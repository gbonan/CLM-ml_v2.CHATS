module MLCanopyFluxesType
  ...
  type, public :: mlcanopy_type
    ...
    real(r8), pointer :: pbeta_lai_canopy(:,:)   ! Parameters for the leaf area density 2-parameter beta distribution (-)
    real(r8), pointer :: pbeta_sai_canopy(:,:)   ! Parameters for the stem area density 2-parameter beta distribution (-)
    ...
    real(r8), pointer :: tref_forcing(:)         ! Air temperature at reference height (K) [interpolated to ML timestep]
    real(r8), pointer :: tref_bef_forcing(:)     ! Air temperature at reference height (K) [previous CLM timestep]
    real(r8), pointer :: tref_cur_forcing(:)     ! Air temperature at reference height (K) [current CLM timestep]
    real(r8), pointer :: tref_next_forcing(:)    ! Air temperature at reference height (K) [next CLM timestep]
    real(r8), pointer :: qref_forcing(:)         ! Specific humidity at reference height (kg/kg) [interpolated to ML timestep]
    real(r8), pointer :: qref_bef_forcing(:)     ! Specific humidity at reference height (kg/kg) [previous CLM timestep]
    real(r8), pointer :: qref_cur_forcing(:)     ! Specific humidity at reference height (kg/kg) [current CLM timestep]
    real(r8), pointer :: qref_next_forcing(:)    ! Specific humidity at reference height (kg/kg) [next CLM timestep]
    real(r8), pointer :: uref_forcing(:)         ! Wind speed at reference height (m/s) [interpolated to ML timestep]
    real(r8), pointer :: uref_bef_forcing(:)     ! Wind speed at reference height (m/s) [previous CLM timestep]
    real(r8), pointer :: uref_cur_forcing(:)     ! Wind speed at reference height (m/s) [current CLM timestep]
    real(r8), pointer :: uref_next_forcing(:)    ! Wind speed at reference height (m/s) [next CLM timestep]
    real(r8), pointer :: pref_forcing(:)         ! Air pressure at reference height (Pa) [interpolated to ML timestep]
    real(r8), pointer :: pref_bef_forcing(:)     ! Air pressure at reference height (Pa) [previous CLM timestep]
    real(r8), pointer :: pref_cur_forcing(:)     ! Air pressure at reference height (Pa) [current CLM timestep]
    real(r8), pointer :: pref_next_forcing(:)    ! Air pressure at reference height (Pa) [next CLM timestep]
    real(r8), pointer :: co2ref_forcing(:)       ! Atmospheric CO2 at reference height (umol/mol) [interpolated to ML timestep]
    real(r8), pointer :: co2ref_bef_forcing(:)   ! Atmospheric CO2 at reference height (umol/mol) [previous CLM timestep]
    real(r8), pointer :: co2ref_cur_forcing(:)   ! Atmospheric CO2 at reference height (umol/mol) [current CLM timestep]
    real(r8), pointer :: co2ref_next_forcing(:)  ! Atmospheric CO2 at reference height (umol/mol) [next CLM timestep]
    real(r8), pointer :: o2ref_forcing(:)        ! Atmospheric O2 at reference height (mmol/mol)
    real(r8), pointer :: swskyb_forcing(:,:)     ! Atmospheric direct beam solar radiation (W/m2) [for numrad wavebands] [interpolated to ML timestep]
    real(r8), pointer :: swskyb_bef_forcing(:,:) ! Atmospheric direct beam solar radiation (W/m2) [for numrad wavebands] [previous CLM timestep]
    real(r8), pointer :: swskyb_cur_forcing(:,:) ! Atmospheric direct beam solar radiation (W/m2) [for numrad wavebands] [current CLM timestep]
    real(r8), pointer :: swskyb_next_forcing(:,:)! Atmospheric direct beam solar radiation (W/m2) [for numrad wavebands] [next CLM timestep]
    real(r8), pointer :: swskyd_forcing(:,:)     ! Atmospheric diffuse solar radiation (W/m2) [for numrad wavebands] [interpolated to ML timestep]
    real(r8), pointer :: swskyd_bef_forcing(:,:) ! Atmospheric diffuse solar radiation (W/m2) [for numrad wavebands] [previous CLM timestep]
    real(r8), pointer :: swskyd_cur_forcing(:,:) ! Atmospheric diffuse solar radiation (W/m2) [for numrad wavebands] [current CLM timestep]
    real(r8), pointer :: swskyd_next_forcing(:,:)! Atmospheric diffuse solar radiation (W/m2) [for numrad wavebands] [next CLM timestep]
    real(r8), pointer :: lwsky_forcing(:)        ! Atmospheric longwave radiation (W/m2) [interpolated to ML timestep]
    real(r8), pointer :: lwsky_bef_forcing(:)    ! Atmospheric longwave radiation (W/m2) [previous CLM timestep]
    real(r8), pointer :: lwsky_cur_forcing(:)    ! Atmospheric longwave radiation (W/m2) [current CLM timestep]
    real(r8), pointer :: lwsky_next_forcing(:)   ! Atmospheric longwave radiation (W/m2) [next CLM timestep]
    ...
    real(r8), pointer :: stflx_air_canopy(:)     ! Canopy air storage heat flux (W/m2)
    real(r8), pointer :: stflx_veg_canopy(:)     ! Canopy biomass storage heat flux (W/m2)
    ...
    real(r8), pointer :: taf_canopy(:)           ! Air temperature at canopy top for ObuFunc (K)
    real(r8), pointer :: qaf_canopy(:)           ! Specific humidity at canopy top for ObuFunc (kg/kg)
    ...
    real(r8), pointer :: dtg_soil(:,:)           ! Change in soil surface temperature over Runge-Kutta step (K)
    ...
    real(r8), pointer :: dtair_profile(:,:,:)    ! Change in canopy layer air temperature over Runge-Kutta step (K)
    real(r8), pointer :: deair_profile(:,:,:)    ! Change in canopy layer vapor pressure over Runge-Kutta step (Pa)
    ...
    real(r8), pointer :: h2ocan_bef_profile(:,:) ! Canopy layer intercepted water for previous timestep (kg H2O/m2)
    real(r8), pointer :: dh2ocan_profile(:,:,:)  ! Change in canopy layer intercepted water over Runge-Kutta step (kg H2O/m2)
    ...
    real(r8), pointer :: dtleaf_leaf(:,:,:,:)    ! Change in leaf temperature over Runge-Kutta step (K)
    ...
    real(r8), pointer :: lwp_bef_leaf(:,:,:)     ! Leaf water potential for previous timestep (MPa)
    real(r8), pointer :: dlwp_leaf(:,:,:,:)      ! Change in leaf water potential over Runge-Kutta step (MPa)
...
contains
  ...
  subroutine InitAllocate (this, bounds)
    ...
    allocate (this%pbeta_lai_canopy    (begp:endp,1:2))                          ; this%pbeta_lai_canopy    (:,:)     = -spval
    allocate (this%pbeta_sai_canopy    (begp:endp,1:2))                          ; this%pbeta_sai_canopy    (:,:)     = -spval
    ...
    allocate (this%tref_bef_forcing    (begp:endp))                              ; this%tref_bef_forcing    (:)       = spval
    allocate (this%tref_cur_forcing    (begp:endp))                              ; this%tref_cur_forcing    (:)       = spval
    allocate (this%tref_next_forcing   (begp:endp))                              ; this%tref_next_forcing   (:)       = spval
    allocate (this%qref_forcing        (begp:endp))                              ; this%qref_forcing        (:)       = spval
    allocate (this%qref_bef_forcing    (begp:endp))                              ; this%qref_bef_forcing    (:)       = spval
    allocate (this%qref_cur_forcing    (begp:endp))                              ; this%qref_cur_forcing    (:)       = spval
    allocate (this%qref_next_forcing   (begp:endp))                              ; this%qref_next_forcing   (:)       = spval
    allocate (this%uref_forcing        (begp:endp))                              ; this%uref_forcing        (:)       = spval
    allocate (this%uref_bef_forcing    (begp:endp))                              ; this%uref_bef_forcing    (:)       = spval
    allocate (this%uref_cur_forcing    (begp:endp))                              ; this%uref_cur_forcing    (:)       = spval
    allocate (this%uref_next_forcing   (begp:endp))                              ; this%uref_next_forcing   (:)       = spval
    allocate (this%pref_forcing        (begp:endp))                              ; this%pref_forcing        (:)       = spval
    allocate (this%pref_bef_forcing    (begp:endp))                              ; this%pref_bef_forcing    (:)       = spval
    allocate (this%pref_cur_forcing    (begp:endp))                              ; this%pref_cur_forcing    (:)       = spval
    allocate (this%pref_next_forcing   (begp:endp))                              ; this%pref_next_forcing   (:)       = spval
    allocate (this%co2ref_forcing      (begp:endp))                              ; this%co2ref_forcing      (:)       = spval
    allocate (this%co2ref_bef_forcing  (begp:endp))                              ; this%co2ref_bef_forcing  (:)       = spval
    allocate (this%co2ref_cur_forcing  (begp:endp))                              ; this%co2ref_cur_forcing  (:)       = spval
    allocate (this%co2ref_next_forcing (begp:endp))                              ; this%co2ref_next_forcing (:)       = spval
    ...
    allocate (this%swskyb_bef_forcing  (begp:endp,1:numrad))                     ; this%swskyb_bef_forcing  (:,:)     = spval
    allocate (this%swskyb_cur_forcing  (begp:endp,1:numrad))                     ; this%swskyb_cur_forcing  (:,:)     = spval
    allocate (this%swskyb_next_forcing (begp:endp,1:numrad))                     ; this%swskyb_next_forcing (:,:)     = spval
    allocate (this%swskyd_forcing      (begp:endp,1:numrad))                     ; this%swskyd_forcing      (:,:)     = spval
    allocate (this%swskyd_bef_forcing  (begp:endp,1:numrad))                     ; this%swskyd_bef_forcing  (:,:)     = spval
    allocate (this%swskyd_cur_forcing  (begp:endp,1:numrad))                     ; this%swskyd_cur_forcing  (:,:)     = spval
    allocate (this%swskyd_next_forcing (begp:endp,1:numrad))                     ; this%swskyd_next_forcing (:,:)     = spval
    allocate (this%lwsky_forcing       (begp:endp))                              ; this%lwsky_forcing       (:)       = spval
    allocate (this%lwsky_bef_forcing   (begp:endp))                              ; this%lwsky_bef_forcing   (:)       = spval
    allocate (this%lwsky_cur_forcing   (begp:endp))                              ; this%lwsky_cur_forcing   (:)       = spval
    allocate (this%lwsky_next_forcing  (begp:endp))                              ; this%lwsky_next_forcing  (:)       = spval
    ...
    allocate (this%stflx_air_canopy    (begp:endp))                              ; this%stflx_air_canopy    (:)       = spval
    allocate (this%stflx_veg_canopy    (begp:endp))                              ; this%stflx_veg_canopy    (:)       = spval
    ...
    allocate (this%dtg_soil            (begp:endp,1:nrk))                        ; this%dtg_soil            (:,:)     = spval
    ...
    allocate (this%dtair_profile       (begp:endp,1:nlevmlcan,1:nrk))            ; this%dtair_profile       (:,:,:)   = spval
    allocate (this%deair_profile       (begp:endp,1:nlevmlcan,1:nrk))            ; this%deair_profile       (:,:,:)   = spval
    ...
    allocate (this%h2ocan_bef_profile  (begp:endp,1:nlevmlcan))                  ; this%h2ocan_bef_profile  (:,:)     = spval
    allocate (this%dh2ocan_profile     (begp:endp,1:nlevmlcan,1:nrk))            ; this%dh2ocan_profile     (:,:,:)   = spval
    ...
    allocate (this%dtleaf_leaf         (begp:endp,1:nlevmlcan,1:nleaf,1:nrk))    ; this%dtleaf_leaf         (:,:,:,:) = spval
    ...
    allocate (this%lwp_bef_leaf        (begp:endp,1:nlevmlcan,1:nleaf))          ; this%lwp_bef_leaf        (:,:,:)   = spval
    allocate (this%dlwp_leaf           (begp:endp,1:nlevmlcan,1:nleaf,1:nrk))    ; this%dlwp_leaf           (:,:,:,:) = spval
