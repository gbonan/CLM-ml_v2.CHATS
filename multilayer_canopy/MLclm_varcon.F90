...
! real(r8) :: phi_psII = 0.85_r8                       ! C3: quantum yield of PS II
  ...
  !----------------------------------------------------!
  real(r8) :: cd = 0.25_r8                             ! Drag coefficient for canopy elements (dimensionless)
  real(r8) :: beta_neutral_max = 0.35_r8               ! Maximum value for beta in neutral conditions
  real(r8) :: cr = 0.3_r8                              ! Parameter to calculate beta_neutral
  real(r8) :: c2 = 0.5_r8                              ! Depth scale multiplier
  real(r8) :: Pr0 = 0.5_r8                             ! Neutral value for Pr (Sc)
  real(r8) :: Pr1 = 0.3_r8                             ! Magnitude of variation of Pr (Sc) with stability
  real(r8) :: Pr2 = 2.0_r8                             ! Scale of variation of Pr (Sc) with stability
  real(r8) :: z0mg = 0.01_r8                           ! Roughness length of ground (m)

  real(r8) :: aH12(3)                                  ! a1 - a3 parameters for Harman (2012, eq. 13)
  data aH12 / 0.89_r8, -0.07_r8, 2.19_r8/              ! values used for CHATS

  ! Limits placed on various variables

  real(r8) :: wind_forc_min = 0.5_r8                   ! Minimum wind speed at forcing height (m/s): Beaufort scale calm air
  real(r8) :: LcL_min = -2._r8                         ! Minimum value for Lc/obu
  real(r8) :: LcL_max = 1._r8                          ! Maximum value for Lc/obu
  real(r8) :: gbh_min = 0.2_r8                         ! Minimum leaf boundary layer conductance for heat (mol/m2/s): for 10 cm leaf at 0.1 m/s
  real(r8) :: ra_max = 500._r8                         ! Maximum aerodynamic resistance for a canopy layer (s/m)
  ...
  real(r8) :: zdtgridM(nZ,1)                           ! Grid of zdt=(z-h)/(h-d) on which psihat is given for momentum
  real(r8) :: dtLgridM(1,nL)                           ! Grid of dtL=(h-d)/L on which psihat is given for momentum
  real(r8) :: psigridM(nZ,nL)                          ! Grid of psihat values for momentum
  real(r8) :: zdtgridH(nZ,1)                           ! Grid of zdt=(z-h)/(h-d) on which psihat is given for heat
  real(r8) :: dtLgridH(1,nL)                           ! Grid of dtL=(h-d)/L on which psihat is given for heat
