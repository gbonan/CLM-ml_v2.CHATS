...
contains
  ...
  subroutine SolarRadiation (bounds, num_filter, filter, mlcanopy_inst)
    ...
    ! See Bonan (2019) Climate Change and Terrestrial Ecosystem Modeling (Chapter 14)
    ! For generality to any layer thickness, see:
    ! Bonan et al. (2021) Agric. For. Met., 306, 108435, doi:j.agrformet.2021.108435, supplemental eqs. (1)-(5)
    !
    ! !USES:
    ...
    do fp = 1, num_filter
       ...
       do ic = ntop(p), nbot(p) , -1
          ...
          ! Leaf optical properties: Constant with height (0) or vary with height (1)
          ! Note: leaf_optics_type is also used in CanopyNitrogenProfile
