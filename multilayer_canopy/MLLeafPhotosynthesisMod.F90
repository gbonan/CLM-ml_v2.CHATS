...
contains
  ...
  subroutine LeafPhotosynthesis (num_filter, filter, il, mlcanopy_inst)
    ...
    !    Bonan et al. (2014) Geosci. Model Dev., 7, 2193-2222, doi:10.5194/gmd-7-2193-2014
    ! For more detailed documentation, see:
    !    Bonan (2019) Climate Change and Terrestrial Ecosystem Modeling (Chapters 11, 12)
    ! For the revised effect of leaf water potential on stomatal conductance, see:
    !   Bonan et al. (2021) Agric. For. Met., 306, 108435, doi:j.agrformet.2021.108435, supplemental eq. (14)
    !
    ! !USES:
    use clm_varcon, only : tfrz
    ...
    g0_BB     => MLpftcon%g0_BB                , &  ! CLMml: Ball-Berry minimum leaf conductance (mol H2O/m2/s)
    g1_BB     => MLpftcon%g1_BB                , &  ! CLMml: Ball-Berry slope of conductance-photosynthesis relationship
    g0_MED    => MLpftcon%g0_MED               , &  ! CLMml: Medlyn minimum leaf conductance (mol H2O/m2/s)
    g1_MED    => MLpftcon%g1_MED               , &  ! CLMml: Medlyn slope of conductance-photosynthesis relationship
    psi50_gs  => MLpftcon%psi50_gs             , &  ! CLMml: Leaf water potential at which 50% of stomatal conductance is lost (MPa)
    shape_gs  => MLpftcon%shape_gs             , &  ! CLMml: Shape parameter for stomatal conductance in relation to leaf water potential (-)
    gsmin_SPA => MLpftcon%gsmin_SPA            , &  ! CLMml: Minimum stomatal conductance (mol H2O/m2/s)
    ...
    vcmax25   => mlcanopy_inst%vcmax25_leaf    , &  ! Leaf maximum carboxylation rate at 25C (umol/m2/s)
    jmax25    => mlcanopy_inst%jmax25_leaf     , &  ! Leaf C3 maximum electron transport rate at 25C (umol/m2/s)
    kp25      => mlcanopy_inst%kp25_leaf       , &  ! Leaf C4 initial slope of CO2 response curve at 25C (mol/m2/s)
    rd25      => mlcanopy_inst%rd25_leaf       , &  ! Leaf respiration rate at 25C (umol CO2/m2/s)
    ...
    btran     => mlcanopy_inst%btran_soil      , &  ! Soil wetness factor for photosynthesis (-)
    ...
    do fp = 1, num_filter
       ...
       do ic = 1, ncan(p)
          ...
          ! High temperature deactivation: The factor "c" scales the
          ! deactivation to a value of 1.0 at 25C
          ...
          if (dpai(p,ic) > 0._r8) then
             ...
             vcmax(p,ic,il) = vcmax25(p,ic,il) * ft(tleaf(p,ic,il), vcmaxha) * fth(tleaf(p,ic,il), vcmaxhd, vcmaxse, vcmaxc) 
             jmax(p,ic,il)  = jmax25(p,ic,il)  * ft(tleaf(p,ic,il), jmaxha)  * fth(tleaf(p,ic,il), jmaxhd, jmaxse, jmaxc)
             rd(p,ic,il)    = rd25(p,ic,il)    * ft(tleaf(p,ic,il), rdha)    * fth(tleaf(p,ic,il), rdhd, rdse, rdc)
             ...
             if (nint(c3psn(patch%itype(p))) == 0) then
                ...
                vcmax(p,ic,il)  = vcmax25(p,ic,il) * t1 / (t2 * t3)
                rd(p,ic,il) = rd25(p,ic,il) * t1 / t4
                kp(p,ic,il) = kp25(p,ic,il) * t1
             end if

             ! Include biochemical effect of soil moisture on photosynthesis
             ! (For now, btran = 1)

             btran(p) = 1._r8
             vcmax(p,ic,il) = vcmax(p,ic,il) * btran(p)

             ! Select parameters for Medlyn or Ball-Berry stomatal models
             ...
             ! Calculate photosynthesis and stomatal conductance without water stress
             ...
             case (1)
                ...
                gs_err = g0(p) + g1(p) * max(anet(p,ic,il), 0._r8) * hs_term / cs(p,ic,il)
             ...
             an_err = (cair(p,ic) - ci(p,ic,il)) / (1._r8 / gbc(p,ic,il) + dh2o_to_dco2 / gs(p,ic,il))
             if (anet(p,ic,il) > 0._r8 .and. abs(anet(p,ic,il)-an_err) > 0.01_r8) then
    ...
    ! Soil moisture adjustment to stomatal conductance and photosynthesis
    ! See Bonan et al. (2021) Agric. For. Met., 306, 108435, doi:j.agrformet.2021.108435, supplemental eq. (14)

    do fp = 1, num_filter
       p = filter(fp)
       do ic = 1, ncan(p)

          ! Save stomatal conductance as the potential conductance in the absence of water stress

          gspot(p,ic,il) = gs(p,ic,il)

          ! Process each canopy layer

          if (dpai(p,ic) > 0._r8) then

             ! Reduce stomatal conductance for water stress, or use potential conductance

             select case (gspot_type)
             case (0)
                fpsi = 1._r8
             case (1)
  ...
  subroutine CiFunc (p, ic, il, mlcanopy_inst, ci_val, ci_dif)
    ...
    use PatchType, only : patch
    use pftconMod, only : pftcon
    use MLclm_varcon, only : qe_c4, vpd_min_MED, dh2o_to_dco2
    use MLclm_varctl, only : gs_type
    ...
    if (dpai(p,ic) > 0._r8) then
       ...
       ! (1) First calculate the metabolic (demand-based) photosynthetic rate
       ...
       call RealizedRate (c3psn(patch%itype(p)), ac(p,ic,il), aj(p,ic,il), ap(p,ic,il), agross(p,ic,il))
       ...
       agross(p,ic,il) = max(agross(p,ic,il), 0._r8)

       ! Net photosynthesis

       anet(p,ic,il) = agross(p,ic,il) - rd(p,ic,il)

       ! CO2 at leaf surface

       cs(p,ic,il) = cair(p,ic) - anet(p,ic,il) / gbc(p,ic,il)
       cs(p,ic,il) = max(cs(p,ic,il), 1._r8)

       ! (2) Now use the stomatal constraint function to calculate gs given An
       ...
       case (1)
          ...
          if (anet(p,ic,il) > 0._r8) then
             term = anet(p,ic,il) / cs(p,ic,il)
             aquad = 1._r8
       ...
       case (0)
          ...
          if (anet(p,ic,il) > 0._r8) then
             ...
             term = dh2o_to_dco2 * anet(p,ic,il) / cs(p,ic,il)
             aquad = 1._r8
       ...
       ! (3) Now use the diffusion (supply-based) photosynthetic rate to calculate Ci

       gleaf = 1._r8 / (1._r8/gbc(p,ic,il) + dh2o_to_dco2/gs(p,ic,il))
       cinew = cair(p,ic) - anet(p,ic,il) / gleaf

       ! ci_dif is the difference between the current Ci and the new Ci

       ci_dif = cinew - ci_val
       if (anet(p,ic,il) < 0._r8) ci_dif = 0._r8
  ...
  subroutine CiFuncGs (p, ic, il, mlcanopy_inst, ci_val)
    ...
    use PatchType, only : patch
    use pftconMod, only : pftcon
    use MLclm_varcon, only : qe_c4, dh2o_to_dco2
    use MLMathToolsMod, only : quadratic
    use MLCanopyFluxesType, only : mlcanopy_type
    !
    ! !ARGUMENTS:
    ...
    if (dpai(p,ic) > 0._r8) then
       ...
       ci_val = cair(p,ic) - anet(p,ic,il) / gleaf
  ...
  subroutine StomataOptimization (p, ic, il, mlcanopy_inst)
    ...
    use PatchType, only : patch
    use MLpftconMod, only : MLpftcon
    use MLclm_varctl, only : gs_solver
  ...
  subroutine StomataEfficiency (p, ic, il, mlcanopy_inst, gs_val, check)
    ...
    ! conductance gs_val, calculate photosynthesis for an increase in
    ! stomatal conductance equal to "delta". The returned value is positive
    ! if this increase produces a change in photosynthesis > iota*vpd*delta.
    ...
    real(r8) :: an_low               ! Leaf photosynthesis at lower gs (umol CO2/m2/s)
    ...
    associate ( &
    iota_SPA    => MLpftcon%iota_SPA            , &  ! CLMml: Stomatal water-use efficiency (umol CO2/ mol H2O)
    ...
    ! vpd is constrained to >= vpd_min_MED to prevent infinite gs at low vpd

    hs = (gbv(p,ic,il)*eair(p,ic) + gs(p,ic,il)*leaf_esat(p,ic,il)) &
       / ((gbv(p,ic,il) + gs(p,ic,il)) * leaf_esat(p,ic,il))
    vpd = max((leaf_esat(p,ic,il) - hs * leaf_esat(p,ic,il)), vpd_min_MED)

    ! Marginal water-use efficiency: check is < 0 when d(An)/d(gs) is < iota * vpd
    ! See Bonan et al. (2014) Geosci. Model Dev., 7, 2193-2222, doi:10.5194/gmd-7-2193-2014, eqs. (A16)-(A18)
  ...
  !-----------------------------------------------------------------------
  subroutine RealizedRate (c3psn, ac, aj, ap, agross)
    !
    ! !DESCRIPTION:
    ! Photosynthesis as the minimum or co-limited rate
    !
    ! !USES:
    use MLclm_varcon, only : colim_c3a, colim_c4a, colim_c4b
    ...
    real(r8), intent(in) :: ac      ! Leaf rubisco-limited gross photosynthesis (umol CO2/m2 leaf/s)
    real(r8), intent(in) :: aj      ! Leaf RuBP regeneration-limited gross photosynthesis (umol CO2/m2 leaf/s)
    real(r8), intent(in) :: ap      ! Leaf product-limited (C3) or CO2-limited (C4) gross photosynthesis (umol CO2/m2 leaf/s)
    real(r8), intent(out) :: agross ! Leaf gross photosynthesis (umol CO2/m2 leaf/s)
    !
    ! !LOCAL VARIABLES:
    real(r8) :: aquad,bquad,cquad   ! Terms for quadratic equations
    real(r8) :: r1,r2               ! Roots of quadratic equation
    ...
    case (0)
       ...
       if (nint(c3psn) == 1) then

          ! No product-limited photosynthesis for C3

          agross = min(ac, aj)

       else if (nint(c3psn) == 0) then
    ...
    case default
       ...
       call endrun (msg=' ERROR: RealizedRate: colim_type not valid')

    end select

  end subroutine RealizedRate
