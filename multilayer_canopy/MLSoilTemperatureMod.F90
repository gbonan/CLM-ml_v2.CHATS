module MLSoilTemperatureMod

  !-----------------------------------------------------------------------
  ! !DESCRIPTION:
  ! Calculate soil temperature. This is simplified from CLM5 to be
  ! compatible with CLMml. It replaces CLM's SoilTemperature called
  ! in clm_drv when CLMml is run uncoupled to CLM. It is not
  ! compatible with CLM and so is not called when CLMml is actively
  ! coupled to CLM.
  !
  ! !USES:
  ...
  waterdiagnosticbulk_inst, waterstatebulk_inst, water_inst, mlcanopy_inst)
    !
    ! !DESCRIPTION:
    ...
    real(r8) :: tssbef   (bounds%begc:bounds%endc,-nlevsno+1:nlevgrnd) ! Soil temperature at beginning of timestep (K)
    ...
    ! on a column so that p = c. This is not true when coupled to  !
    ! CLM. Here, pfilter is used to make p -> c for use with gsoi. !
    ...
    temperature_inst, waterdiagnosticbulk_inst, waterstatebulk_inst, water_inst, soilstate_inst)
    ...
    do fc = 1, num_nolakec
       ...
       if (abs(gsoi(pfilter(c))-edif) >= 1.e-06_r8) then
          call endrun (msg='ERROR: MLSoilTemperatureMod: soil temperature energy conservation error')
       end if
  ...
  tk, cv, tk_h2osfc, temperature_inst, waterdiagnosticbulk_inst, waterstatebulk_inst, water_inst, soilstate_inst)
    !
    ! !DESCRIPTION:
    ! Calculation of thermal conductivities and heat capacities of snow/soil layers
    ! as adapted from CLM5
    !
    ! !USES:
    ...
    h2osfc      =>  waterstatebulk_inst%h2osfc_col           , & ! Input:  [real(r8) (:)]    surface water (mm H2O)                        
    h2osno      =>  water_inst%h2osno_col                    , & ! Input:  [real(r8) (:)]    snow water (mm H2O)                     
