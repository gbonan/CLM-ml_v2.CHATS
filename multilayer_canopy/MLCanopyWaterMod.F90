module MLCanopyWaterMod
  ...
  !-----------------------------------------------------------------------
  ! !DESCRIPTION:
  ! Update canopy water. These routines are based on CLM5/6
  !
  ! !USES:
...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine CanopyWettedFraction (num_filter, filter, mlcanopy_inst)
    !
    ! !DESCRIPTION:
  ...
  subroutine CanopyInterception (num_filter, filter, mlcanopy_inst)
    ...
    !
    ! !USES:
    use MLclm_varcon, only : dewmx, interception_fraction
    ...
    do fp = 1, num_filter
       ...
       ! Fraction of precipitation that is intercepted (as in CLM5)
       ...
       do ic = 1, ncan(p)
          ...
          if (dpai(p,ic) > 0._r8) then
             ...
             h2ocan(p,ic) = h2ocan_bef(p,ic) + qflx_intr(p) * dtime / float(n)
