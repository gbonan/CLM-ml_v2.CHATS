...
contains
  ...
  subroutine PlantResistance (num_filter, filter, mlcanopy_inst)
    ...
    associate ( &
                                                  ! *** Input ***
    gplant_SPA => MLpftcon%gplant_SPA        , &  ! CLMml: Stem (xylem-to-leaf) hydraulic conductance (mmol H2O/m2 leaf area/s/MPa)
    ...
    do fp = 1, num_filter
       ...
       do ic = 1, ncan(p)
          ...
          if (dpai(p,ic) > 0._r8) then
             ...
             ! See Bonan et al. (2014) Geosci. Model Dev., 7, 2193-2222, doi:10.5194/gmd-7-2193-2014, eq. (A22)
...
!            rplant = zs(p,ic) / gplant_SPA(patch%itype(p))       ! gplant_SPA is conductivity (mmol H2O/m/s/MPa)
             ...
             ! See Bonan et al. (2014) Geosci. Model Dev., 7, 2193-2222, doi:10.5194/gmd-7-2193-2014, eq. (A21)
  ...
  soilstate_inst, waterstatebulk_inst, mlcanopy_inst)
    ...
    associate ( &
                                                               ! *** Input ***
    root_radius_SPA  => MLpftcon%root_radius_SPA          , &  ! CLMml: Fine root radius (m)
    root_density_SPA => MLpftcon%root_density_SPA         , &  ! CLMml: Fine root density (g biomass / m3 root)
    root_resist_SPA  => MLpftcon%root_resist_SPA          , &  ! CLMml: Hydraulic resistivity of root tissue (MPa.s.g/mmol H2O)
    ...
    ! See Bonan et al. (2014) Geosci. Model Dev., 7, 2193-2222, doi:10.5194/gmd-7-2193-2014
    ...
    do fp = 1, num_filter
       ...
       do j = 1, nlayers
          ...
          ! Soil-to-root resistance (MPa.s.m2/mmol H2O): eq. (A23)

          soilr1 = log(root_dist/root_radius_SPA(patch%itype(p))) / (2._r8 * pi * root_length_density * dz(c,j) * hk)

          ! Root-to-stem resistance (MPa.s.m2/mmol H2O): eq. (A24)
          ...
          !
          ! WARNING: evap uses the original SPA plant hydraulic implementation with
          ! minlwp_SPA. The current implementation of plant hydraulics does not use minlwp_SPA.
          ! See eq. (A26)
       ...
       ! Belowground resistance: resistance = 1 / conductance. See eq. (A25)

       rsoil(p) = lai(p) / rsoil(p)

       ! Weighted soil water potential (MPa) and fractional uptake from soil layers
       ! See eqs. (A26)-(A28) and the preceeding warning
  ...
  subroutine LeafWaterPotential (num_filter, filter, il, mlcanopy_inst)
    ...
    associate ( &
                                                   ! *** Input ***
    capac_SPA   => MLpftcon%capac_SPA         , &  ! CLMml: Plant capacitance (mmol H2O/m2 leaf area/MPa)
    ncan        => mlcanopy_inst%ncan_canopy  , &  ! Number of aboveground layers
    psis        => mlcanopy_inst%psis_soil    , &  ! Weighted soil water potential (MPa)
    dpai        => mlcanopy_inst%dpai_profile , &  ! Canopy layer plant area index (m2/m2)
    zs          => mlcanopy_inst%zs_profile   , &  ! Canopy layer height for scalar concentration and source (m)
    ...
    ! See Bonan et al. (2014) Geosci. Model Dev., 7, 2193-2222, doi:10.5194/gmd-7-2193-2014, eqs. (A19)-(A20)
