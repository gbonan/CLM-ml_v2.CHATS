module TowerMetMod
  ...
  private :: TowerMetCO2       ! Atmospheric CO2 concentration
  private :: TowerMetSolarRad  ! Partition solar radiation into component fluxes
  private :: TowerMetEmiss     ! Atmospheric emissivity
  !-----------------------------------------------------------------------

contains

  !-----------------------------------------------------------------------
  function TowerMetCO2 () result (co2_ppm)
    !
    ! !DESCRIPTION:
    ...
    !
    ! !LOCAL VARIABLES:
    real(r8) :: co2_ppm            ! Atmospheric CO2 (ppm)
    !---------------------------------------------------------------------
...
!   co2_ppm = 367._r8
  ...
  end function TowerMetCO2

  !-----------------------------------------------------------------------
  subroutine TowerMetSolarRad (fsds, forc_solad_vis, forc_solai_vis, &
  forc_solad_nir, forc_solai_nir)
    !
    ! !DESCRIPTION:
    ! Partition solar radiation into direct and diffuse components each
    ! with visible and near-infrared wavebands
    !
    ! !USES:
    !
    ! !ARGUMENTS:
    implicit none
    real(r8), intent(in)  :: fsds            ! Solar radiation (W/m2)
    real(r8), intent(out) :: forc_solad_vis  ! Direct beam radiation, visible (W/m2)
    real(r8), intent(out) :: forc_solai_vis  ! Diffuse radiation, visible (W/m2)
    real(r8), intent(out) :: forc_solad_nir  ! Direct beam radiation, near-infrared (W/m2)
    real(r8), intent(out) :: forc_solai_nir  ! Diffuse radiation, near-infrared (W/m2)
    !
    ! !LOCAL VARIABLES:
    ...
    real(r8) :: rnir                         ! Direct beam fraction of nir solar radiation
    ...
    ! Total solar radiation is split equally to the visible and
    ! near-infrared wavebands and is then split into direct beam and diffuse
    ...
    forc_solai_vis = fsds_vis * (1._r8 - rvis)
    forc_solad_nir = fsds_nir * rnir
    forc_solai_nir = fsds_nir * (1._r8 - rnir)

  end subroutine TowerMetSolarRad

  !-----------------------------------------------------------------------
  function TowerMetEmiss (eair, tair) result (emiss)
    !
    ! !DESCRIPTION:
    ! Atmospheric emissivity (if longwave radiation is missing)
    !
    ! !USES:
    !
    ! !ARGUMENTS:
    implicit none
    real(r8), intent(in) :: eair        ! Atmospheric vapor pressure (Pa)
    real(r8), intent(in) :: tair        ! Atmospheric temperature (K)
    !
    ! !LOCAL VARIABLES:
    real(r8) :: emiss                   ! Atmospheric emissivity
    !---------------------------------------------------------------------

    emiss = 0.7_r8 + 5.95e-05_r8 * 0.01_r8 * eair * exp(1500._r8/tair)

  end function TowerMetEmiss

  !-----------------------------------------------------------------------
  subroutine TowerMetCurr (ncfilename, strt, it, begp, endp, atm2lnd_inst, &
  wateratm2lndbulk_inst, frictionvel_inst)
    !
    ! !DESCRIPTION:
    ! Read atmospheric forcing variables for current timestep
    !
    ! !USES:
    ...
    type(wateratm2lndbulk_type), intent(out) :: wateratm2lndbulk_inst
    ...
    forc_solad => atm2lnd_inst%forc_solad_downscaled_col        , &  ! CLM: Atmospheric direct beam radiation (W/m2)
    ...
    do p = begp, endp
       ...
       ! Incoming solar direct beam and diffuse radiation. Solar
       ! radiation is split to visible and near-infrared wavebands
       ! and is then split into direct beam and diffuse components.
       !
       ! NOTE: forc_solad is at column (c) and forc_solai is at grid cell (g)

       fsds = max(fsds_loc(1,1,1), 0._r8)
       call TowerMetSolarRad (fsds, forc_solad(c,ivis), forc_solai(g,ivis), forc_solad(c,inir),forc_solai(g,inir))
       ...
       if (nint(forc_lwrad(c)) == -999) then
          emiss = TowerMetEmiss(eair, forc_t(c))
          forc_lwrad(c) = emiss * sb * forc_t(c)**4
       end if

       ! CO2: umol/mol -> Pa and O2: mol/mol -> Pa

       forc_pco2(g) = (TowerMetCO2() / 1.e06_r8) * forc_pbot(c)
  ...
  !-----------------------------------------------------------------------
  subroutine TowerMetNext (ncfilename, strt, begp, endp, mlcanopy_inst)
    !
    ! !DESCRIPTION:
    ! Read atmospheric forcing variables for next timestep. This is
    ! needed for the 3-point time interpolation of atmospheric forcing
    ! from the CLM timestep to the multilayer canopy timestep. These
    ! are multilayer canopy variables not CLM variables.
    !
    ! !USES:
    use clm_varcon, only : sb
    use clm_varpar, only : ivis, inir
    ...
    ! Assign these variables to the appropriate multilayer canopy variables

    do p = begp, endp

       uref_next(p) = uref_loc(1,1,1)
       tref_next(p) = tref_loc(1,1,1)
       qref_next(p) = qref_loc(1,1,1)
       pref_next(p) = pref_loc(1,1,1)
       lwsky_next(p) = flds_loc(1,1,1)

       fsds = max(fsds_loc(1,1,1), 0._r8)
       call TowerMetSolarRad (fsds, swskyb_next(p,ivis), swskyd_next(p,ivis), swskyb_next(p,inir),swskyd_next(p,inir))

       ! Set values that are missing (same as TowerMetCurr)

       if (nint(pref_next(p)) == -999) pref_next(p) = 101325._r8

       forc_rh = rhref_loc(1,1,1)
       if (nint(forc_rh) /= -999) then
          call SatVap (tref_next(p), esat, desat)
          eair = (forc_rh / 100._r8) * esat
          qref_next(p) = mmh2o / mmdry * eair / (pref_next(p) - (1._r8 - mmh2o/mmdry) * eair)
       else if (nint(qref_next(p)) /= -999) then
          eair = qref_next(p) * pref_next(p) / (mmh2o / mmdry + (1._r8 - mmh2o / mmdry) * qref_next(p))
       else
          call endrun (msg=' TowerMetNext error: rhref and qref not valid')
       end if

       if (nint(lwsky_next(p)) == -999) then
          emiss = TowerMetEmiss(eair, tref_next(p))
          lwsky_next(p) = emiss * sb * tref_next(p)**4
       end if

       ! CO2: Here CO2 is ppm (as in multilayer code)
