module clmDataMod
  ...
  private :: readCLMsoil          ! Read soil moisture from CLM netcdf history file
  private :: readSoilWatFactor    ! Read soil moisture adjustment from a netcdf file
  !-----------------------------------------------------------------------

contains

  !-----------------------------------------------------------------------
  subroutine clmData (fin_clm, fin_soil_adjust, strt, begp, endp, begc, endc, &
  soilstate_inst, waterstatebulk_inst, canopystate_inst, surfalb_inst)
    ...
    ! !ARGUMENTS:
    implicit none
    character(len=*), intent(in) :: fin_clm         ! CLM netcdf filename
    character(len=*), intent(in) :: fin_soil_adjust ! Soil moisture adjustment netcdf file name
    ...
    type(waterstatebulk_type), intent(out) :: waterstatebulk_inst
    ...
    call readCLMveg (fin_clm, strt, elai_loc, esai_loc)
    ...
    call readCLMsoil (fin_clm, strt, h2osoi_clm45, h2osoi_clm50)
    ...
    do c = begc, endc
       ...
       ! Overwrite CLM soil moisture

       if (nlev_soil_adjust > 0) then
          call readSoilWatFactor (fin_soil_adjust, strt, h2osoi_factor_loc)
          h2osoi_factor = h2osoi_factor_loc(1,1,1)
          do j = 1, nlev_soil_adjust
             h2osoi_vol(c,j) = h2osoi_vol(c,j) * h2osoi_factor
          end do
       end if
  ...
  !-----------------------------------------------------------------------
  subroutine readCLMveg (ncfilename, strt, elai_mod, esai_mod)
    !
    ! !DESCRIPTION:
  ...
  !-----------------------------------------------------------------------
  subroutine readSoilWatFactor (ncfilename, strt, h2osoi_factor_loc)
    !
    ! !DESCRIPTION:
    ...
    character(len=*), intent(in) :: ncfilename         ! netcdf filename
    integer,  intent(in)  :: strt                      ! Time slice of data to retrieve
    real(r8), intent(out) :: h2osoi_factor_loc(1,1,1)  ! Soil moisture factor
    !
    ! !LOCAL VARIABLES:
    integer :: ncid                                ! netcdf file ID
    integer :: status                              ! Function return status
    ...
    ! Dimensions in FORTRAN are in column major order: the first array index varies the most rapidly.
    ! In NetCDF file the dimensions appear in the opposite order.
 
    start1 = (/ strt /)
    count1 = (/ 1 /)

    ! factor(ntime): soil moisture factor

    status = nf_inq_varid(ncid, "FACTOR", varid)
    if (status /= nf_noerr) call handle_err(status, "FACTOR")

    status = nf_get_vara_double(ncid, varid, start1, count1, h2osoi_factor_loc)
    if (status /= nf_noerr) call handle_err(status, "h2osoi_factor_loc")
